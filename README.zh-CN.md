...
- **🏠 首页「快速开始」引导条**（完成前常驻）：4 步清单 —— ① 配置 LLM → ② 导入学习资料 → ③ 开始考核（理论 / 实战 / 面试）→ ④ 查看能力画像。每步实时显示状态（✅ 已完成 / 🔒 待完成），并且**当前最该做的一步会自动高亮并带「去完成 →」按钮**。
- **🎬 首次自动播放的高亮引导 Tour**（仅首次，之后可随时点「👀 新手引导演示」重看）：聚光灯遮罩 + 步骤气泡带你把 4 步走一遍，点一下即可直接跳到对应页面。
