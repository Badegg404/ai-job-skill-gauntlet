  ...
  render(() => {
    ...
    <div class="exam-hero" style="margin-bottom:26px">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid var(--border)">
        <div style="font-size:12px;color:var(--accent-2);font-family:var(--mono);letter-spacing:1px;display:flex;align-items:center;gap:7px"><span style="color:#ff3df0">&gt;_</span> SYSTEM // 能力试炼 · Skill Gauntlet</div>
...
function exportProfile() {
  ...
  const lines = [];
  lines.push("# 能力试炼 · Skill Gauntlet 能力报告");
  lines.push("");
  lines.push("> 由「能力试炼 · Skill Gauntlet」自动生成 · " + new Date().toISOString().slice(0, 16).replace("T", " "));
  lines.push("");
  lines.push("## 基础水平");
  ...
  a.download = "能力试炼报告.md";
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  showToast("✅ 已导出「能力试炼报告.md」");
}
...
async function init() {
  ...
  if (!LLM_KEY && !localStorage.getItem("examCenter.llmGuided")) {
    ...
    setTimeout(() => {
      showModal({
        icon: "🤖",
        title: "欢迎使用 能力试炼 · Skill Gauntlet",
