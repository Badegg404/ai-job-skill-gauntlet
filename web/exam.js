  ...
  render(() => {
    ...
    <div class="exam-hero" style="margin-bottom:26px">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid var(--border)">
        <div style="font-size:12px;color:var(--accent-2);font-family:var(--mono);letter-spacing:1px;display:flex;align-items:center;gap:7px"><span style="color:#ff3df0">&gt;_</span> SYSTEM // AI 岗位能力评估终端</div>
...
function exportProfile() {
  ...
  const lines = [];
  lines.push("# AI 岗位能力评估报告");
  lines.push("");
  lines.push("> 由「AI 岗位面试能力评估系统」自动生成 · " + new Date().toISOString().slice(0, 16).replace("T", " "));
  lines.push("");
  lines.push("## 基础水平");
  ...
  a.download = "AI岗位能力评估报告.md";
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  showToast("✅ 已导出「AI岗位能力评估报告.md」");
}
