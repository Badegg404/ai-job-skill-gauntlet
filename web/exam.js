...
function goHome() {
  ...
  const modeCards = [
    ...
    { icon: "🛠️", title: "实战考核", desc: "跨章节综合 · 场景决策 + 代码运行对比", tag: "综合", onClick: "startExam('practical')", disabled: isEmpty },
  ...
  render(() => {
    ...
    <div class="exam-hero" style="margin-bottom:26px">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid var(--border)">
        <div style="font-size:12px;color:var(--accent-2);font-family:var(--mono);letter-spacing:1px;display:flex;align-items:center;gap:7px"><span style="color:#ff3df0">&gt;_</span> SYSTEM // AI 能力试炼 · Skill Gauntlet</div>
...
function showImportPanel() {
  ...
  if (!LLM_KEY) {
    showModal({
      icon: "🤖",
      title: "导入资料需要 LLM",
      text: "导入时由 LLM 出题（理论客观题 + 实战场景题，共 12 道），并为对应岗位提炼面试参考题。未配置 LLM 无法生成考核题。请先配置 API Key（支持 DeepSeek、阿里百炼等 OpenAI 兼容接口）。",
      actions: [
...
function exportProfile() {
  ...
  const lines = [];
  lines.push("# AI 能力试炼 · Skill Gauntlet 能力报告");
  lines.push("");
  lines.push("> 由「AI 能力试炼 · Skill Gauntlet」自动生成 · " + new Date().toISOString().slice(0, 16).replace("T", " "));
  lines.push("");
  lines.push("## 基础水平");
  ...
  a.download = "AI能力试炼报告.md";
  document.body.appendChild(a);
  a.click();
  a.remove();
  setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  showToast("✅ 已导出「AI能力试炼报告.md」");
}
...
async function init() {
  ...
  if (!LLM_KEY && !localStorage.getItem("examCenter.llmGuided")) {
    ...
    setTimeout(() => {
      showModal({
        icon: "🤖",
        title: "欢迎使用 AI 能力试炼 · Skill Gauntlet",
