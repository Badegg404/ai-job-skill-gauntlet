...
/* 导入资料时的出题：生成 20 道考核题（理论 10 + 实战 10，理论客观题 + 代码实战客观题）；前端单轮调用后去重入库。
...
请生成 20 道题，覆盖两大考核维度（理论 10 + 实战 10）——实战题多生成一些，作为考核抽题的备选池（避免考核时反复抽到同样的题）：

一、理论维度（dimension 填 "theory"）—— 考察概念/原理的客观掌握，客观题为主（选择题、判断题尽量多出）：
  5 道概念辨析选择题（4 选项，correctIndex 为 0-3，题干基于核心概念，覆盖不同知识点、不要雷同）
  3 道判断题（true_false，correctAnswer 填 "对" 或 "错"）：题干陈述本身必须语义自洽、可直接判定真伪——题干说法正确就填「对」，说法错误就填「错」，并在 explanation 说明对错原因。出「错」题时，请在题干里写一个「本身错误」的技术说法（如把概念/机制说反），禁止用「不符合课程案例 / 与 demo 不同」这类题外理由判定对错（判断题只考陈述本身的真伪，不考是否与某案例一致）。
...
二、实战维度（dimension 填 "practical"）—— 基于课程【真实代码】的代码实战客观题（type 用 "practical"，practical.compareMode 填 "code_choice"）：
  8 道，必须引用上面「代码文件」里的真实代码（真实文件名/函数名/代码片段），类型多样化（8 道不要全同一种，5 种类型尽量都覆盖，同一段代码也可以从不同角度出题但要保证不雷同），在以下 5 种中选：
