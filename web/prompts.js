...
请生成 10 道代码实战题——大部分为代码客观题（code_choice，8 道），少量为写代码任务题（llm_code，2 道），类型多样化、不要雷同：

二、实战维度（dimension 填 "practical"）—— 基于课程【真实代码】：
  A. 8 道代码客观题（type 用 "practical"，practical.compareMode 填 "code_choice"）：必须引用上面「代码文件」里的真实代码（真实文件名/函数名/代码片段），类型多样化（8 道不要全同一种，5 种类型尽量都覆盖，同一段代码也可以从不同角度出题但要保证不雷同），在以下 5 种中选：
  ...
  B. 2 道写代码任务题（practical.compareMode 填 "llm_code"）：从上面某个代码文件的【实现逻辑】出发，问用户「这块代码该如何写/如何实现」，并在题干中提示参考实现所在文件（如「提示：参考实现可在 {文件名} 中找到」）；task 描述具体编码任务（轻量函数/逻辑题，不要完整大项目），referenceAnswer 给参考实现，scoringPoints 给 2-4 条评分要点。
